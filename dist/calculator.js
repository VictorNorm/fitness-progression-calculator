...
function calculateProgression(data, programType, userSettings = DEFAULT_EQUIPMENT_SETTINGS) {
    ...
    switch (data.rating) {
        case 1: {
            ...
            // Apply percentage cap to prevent excessive increases
            const cappedVeryEasyWeight = applyPercentageCap(data.weight, veryEasyTargetWeight, data.is_compound, data.equipment_type, userSettings);
            // Round to valid equipment increments
            const roundedVeryEasyWeight = roundToIncrementMultiple(cappedVeryEasyWeight, data.equipment_type, userSettings);
            ...
            const cappedEasyWeight = applyPercentageCap(data.weight, easyTargetWeight, data.is_compound, data.equipment_type, userSettings);
            const roundedEasyWeight = roundToIncrementMultiple(cappedEasyWeight, data.equipment_type, userSettings);
            return {
        ...
        case 5: {
            ...
            const roundedTooHardWeight = roundToIncrementMultiple(tooHardWeight, data.equipment_type, userSettings);
            return {
